...
__global__ void avgPoolCu(const  float* __restrict__ inputData, float * outData, int sample, int feature,
				...
				for (int forCha = 0; forCha < chanal; forCha++){
					...
					for (int poolX = 0; poolX < poolScale; poolX++){
						for (int poolY = 0; poolY < poolScale; poolY++){
							...
							if (inputData[inIndex] > 0.0)
								maxPiks += inputData[inIndex];
						}
					}
					outData[outIndex] = maxPiks / (poolScale*poolScale);
				}
			}
		}
	}
...
void poolAvg2d(Layer * inputLayer, Layer* outLayer, int poolScale){
	...
	if (inSam == outSam && inFea == outFea  &&  inCha == outCha){
		if (inHe == outHe*poolScale  &&  inWi == outWi*poolScale){
			...
			avgPoolCu << <gridsize, bloksize >> >(inputData, outputData, inSam, inFea, inCha, outHe, outWi, poolScale);
		}
